...
########################
    def validate_accountpreference(self,field):
        username = field.data 
        if username [0] == "@":
            raise ValidationError ("Twitter Account cannot start with an @ sign")
    
    def validate_accountpreference(self,field):
        username_length = field.data 
        if len (username_length.split(" ")) < 1:
            raise ValidationError ("Twitter Account must be at least 1 word")
